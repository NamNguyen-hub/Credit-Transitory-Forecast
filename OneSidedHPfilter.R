...
filepath2 = sprintf('/Credit_HPfilter_%s.txt',country)
...
df = df[varlist]
credit <- xts(df[,-c(1)], order.by=as.Date(df[,"date"], "%Y-%m-%d"))
...
df1 = df1[varlist]
credit_BIS <- xts(df1[,-c(1)], order.by=as.Date(df1[,"date"], "%Y-%m-%d"))
credit_BIS <- ts_ts(credit_BIS)


df1 <- df %>%
  filter(grepl("^(C)", cg_dtype))
# df = subset(df, date > as.Date(startdate)) # Limit series data to after 1990
# df = subset(df, date < as.Date(enddate)) # Limit series data to before 2020
varlist = c("date", "obs_value")
df1 = df1[varlist]
credit_BIS_gap <- xts(df1[,-c(1)], order.by=as.Date(df1[,"date"], "%Y-%m-%d"))
credit_BIS_gap <- ts_ts(credit_BIS_gap)
...
out1=filterHP(credit_BIS, lambda=400000)[,"cycle"]
out2=filterHP(credit0, lambda=40000)[,"cycle"]

plot(hpfilter(credit_BIS,freq=400000)$cycle, ylim=c(-15,15), col=1,ylab="", main="US: Estimated Cyclical components")
lines(out1, col=2)
lines(credit_BIS_gap, col=3)
lines(out2, col=4)
legend("topleft",legend=c("HPfilter 2 sided", "HPfilter 1 sided", "credit_BIS_gap", "credit_to_household"),
       col=1:4,lty=rep(1,4),ncol=2)
